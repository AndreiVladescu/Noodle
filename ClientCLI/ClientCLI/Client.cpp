...
{
	vector<string> recvMessageVector;
	string recvString;

	if (!GetString(recvString)) {
		return false;
	}

	recvMessageVector = PARSER->unparseString(recvString, '$');

	int uRole = atoi(recvMessageVector[6].c_str());

	switch (uRole) {
	case 1: { // Student
		user = FactoryUser::createUserStudent();
		break;
	}
	case 2: {// Professor
		user = FactoryUser::createUserProfesor();
		break;
	}
	case 3: {// Admin
	...
	default:
		user = FactoryUser::createUserStudent(); //if i write no default case at all the compiler screams
	}

	user->setConnectionData(recvMessageVector[2],
		recvMessageVector[3],
		recvMessageVector[4],
		recvMessageVector[5],
		atoi(recvMessageVector[1].c_str()),
		atoi(recvMessageVector[6].c_str())
	);

	if (atoi(recvMessageVector[6].c_str()) == 1) {
		dynamic_cast<Student*>(user)->setStudyGroup(recvMessageVector[7]);
		dynamic_cast<Student*>(user)->setStudyYear(atoi(recvMessageVector[8].c_str()));
	}
...
void Client::printUserInfo()
{
	...
	switch (user->getUserRole()) {
		case (1): {
			cout << "Student" << endl;
			cout << "Study Group : " << dynamic_cast<Student*>(user)->getStudyGroup() << endl;
			cout << "Study Year : " << dynamic_cast<Student*>(user)->getStudyYear() << endl;
			break;
		}
		case(2): {
			cout << "Professor" << endl;
...
//bool Client::SendDirectMessage(const string& value)
//{
